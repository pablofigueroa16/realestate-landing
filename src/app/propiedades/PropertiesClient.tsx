...
import { CityProperty } from "@/utils/getAllProperties";
      ...
      >
        ...
        {images.length > 1 && (
          <div className={`absolute inset-0 flex items-center justify-between px-2 transition-opacity duration-300 z-20 ${isHovered ? "opacity-100" : "opacity-0"}`}>
            <button onClick={prevImage} className="w-8 h-8 rounded-full bg-white/30 backdrop-blur-md flex items-center justify-center hover:bg-white/50 transition-colors">
              <ChevronLeft size={16} className="text-white" />
            </button>
            <button onClick={nextImage} className="w-8 h-8 rounded-full bg-white/30 backdrop-blur-md flex items-center justify-center hover:bg-white/50 transition-colors">
        ...
        {images.length > 1 && (
          <div className="absolute bottom-3 left-1/2 -translate-x-1/2 flex gap-1.5 z-20">
            {images.slice(0, 5).map((_, idx) => (
              <div key={idx} className={`h-1.5 rounded-full transition-all duration-300 shadow-sm ${currentIndex === idx ? "w-4 bg-white" : "w-1.5 bg-white/50"}`} />
            ))}
          </div>
        )}
      </div>
...
interface Props {
  initialProperties: CityProperty[];
}
...
export default function PropertiesClient({ initialProperties }: Props) {
  ...
  const cities = useMemo(() => {
    const uniqueCities = [...new Set(initialProperties.map((p) => p.citySlug))];
    return uniqueCities.map((slug) => ({
      slug,
      name: initialProperties.find((p) => p.citySlug === slug)?.city || slug,
    }));
  }, [initialProperties]);

  const filteredProperties = useMemo(() => {
    let result = [...initialProperties];
    if (selectedCity !== "all") result = result.filter((p) => p.citySlug === selectedCity);
    if (searchQuery.trim()) {
    ...
    if (sortBy === "name") result.sort((a, b) => a.hero.title.localeCompare(b.hero.title));
    return result;
  }, [initialProperties, selectedCity, searchQuery, sortBy]);
      ...
      <section className="pt-32 pb-16 bg-gray-900">
        <div className="container mx-auto px-6 md:px-12 lg:px-20">
          <div className="max-w-3xl">
            ...
            <p className="text-gray-400 text-lg leading-relaxed">{t("properties_page.subtitle")}</p>
          </div>
        </div>
      </section>
          ...
          <div className="flex flex-col md:flex-row gap-4 items-center">
            <div className="relative w-full md:w-80">
              ...
              <input
                ...
                className="w-full pl-12 pr-4 py-3 rounded-full border border-gray-200 focus:outline-none focus:border-gray-400 transition-colors text-gray-900 bg-white"
              />
            </div>
            <div className="relative w-full md:w-48">
              ...
              >
                <span className="text-gray-700">
                  {selectedCity === "all" ? t("properties_page.all_cities") : cities.find((c) => c.slug === selectedCity)?.name}
                </span>
              ...
              {isCityDropdownOpen && (
                <div className="absolute top-full left-0 mt-2 w-full bg-white rounded-xl shadow-xl border border-gray-100 overflow-hidden py-1 z-50">
                  <button onClick={() => { setSelectedCity("all"); setIsCityDropdownOpen(false); }} className={`w-full px-4 py-2 text-left text-sm hover:bg-gray-50 transition-colors ${selectedCity === "all" ? "font-bold text-black" : "text-gray-700"}`}>
                    {t("properties_page.all_cities")}
                  </button>
                  {cities.map((city) => (
                    <button key={city.slug} onClick={() => { setSelectedCity(city.slug); setIsCityDropdownOpen(false); }} className={`w-full px-4 py-2 text-left text-sm hover:bg-gray-50 transition-colors ${selectedCity === city.slug ? "font-bold text-black" : "text-gray-700"}`}>
                      {city.name}
                    </button>
                  ))}
                </div>
            ...
            <div className="relative w-full md:w-48">
              ...
              {isSortDropdownOpen && (
                <div className="absolute top-full left-0 mt-2 w-full bg-white rounded-xl shadow-xl border border-gray-100 overflow-hidden py-1 z-50">
                  <button onClick={() => { setSortBy("newest"); setIsSortDropdownOpen(false); }} className={`w-full px-4 py-2 text-left text-sm hover:bg-gray-50 transition-colors ${sortBy === "newest" ? "font-bold text-black" : "text-gray-700"}`}>
                    {t("properties_page.sort_newest")}
                  </button>
                  <button onClick={() => { setSortBy("name"); setIsSortDropdownOpen(false); }} className={`w-full px-4 py-2 text-left text-sm hover:bg-gray-50 transition-colors ${sortBy === "name" ? "font-bold text-black" : "text-gray-700"}`}>
        ...
        <div className="container mx-auto px-6 md:px-12 lg:px-20">
          {filteredProperties.length > 0 ? (
            ...
            <div className="text-center py-20">
              ...
              <button onClick={clearFilters} className="px-6 py-3 bg-gray-900 text-white rounded-full hover:bg-gray-800 transition-colors font-medium">
